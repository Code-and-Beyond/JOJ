...
const googleAuth = async (req: Request, res: Response, next: NextFunction) => {
    let { username, token, role } = req.body;
    let url =
    ...
    try {
        ...
        if (result.rows.length < 1) {
            ...
            await Query(
                client,
                'INSERT INTO users (uid, username, role, fname, lname, fullname, image, logins, lastlog) VALUES ($1, $2, $3, $4, $5, $6, $7, 1, CURRENT_TIMESTAMP)',
                [
                    uid,
                    googleData.email,
                    role,
